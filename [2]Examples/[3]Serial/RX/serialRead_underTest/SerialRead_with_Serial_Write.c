/*
*Description: How to setup serial port in atmel studio and atmega328p-pu micro controller.
...
//================================== RX END=============================================
	...
	sw("now watch what happens when you type the number 1\n\r ");
	
	while(1)
	{
		
		if(udr0 == '1')//udr0 variable always gets updated when a char is received
		{
			udr0 = '\0'; /* Never forget to insert NULL in udr0 after it is used*/
			sw("TCCR0A is ");
			swn(TCCR0A,10,1);
			_delay_ms(1000);
		}
