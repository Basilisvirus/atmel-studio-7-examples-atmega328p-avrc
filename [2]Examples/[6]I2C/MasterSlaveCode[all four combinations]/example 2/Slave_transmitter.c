...
void TWI_Match_ACK(){//wait for address reference
	while((TWSR & (0xF8))!= 0xA8)//while Own SLA+R has NOT been received; and ACK has NOT been returned
	{
...
void TWI_Write_Data(uint8_t Data){
	...
	while((TWSR & (0xF8))!= 0xB8);//byte in TWDR has been transmitted; ACK has been received
}
...
int main(void)
{
	...
	sw("slave Started\n\r");
	TWI_Match_ACK();
	sw("match acknowledged\n\r");	
	
	while(1)
	{	
		TWI_Write_Data(3);
		sw("Data sent, ack received\n\r");
		_delay_ms(500);
	}//while(1)
}//int main()
